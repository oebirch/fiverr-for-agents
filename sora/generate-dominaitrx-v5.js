// Generated videos: video_dcc8d993.mp4, video_2814b7a2.mp4
...
async function generateDominaitrxV5Ad() {
  try {
    console.log('🎬 Generating dominAItrx V5 Video Ad\n');
...
SCENE 1 (1 second MAX): Authentic 80s VHS infomercial opening. Scan lines, slight static, VHS tracking artifacts. Bright gradient background (pink to blue). Large bold text: "dominAItrx" - use clean professional retro TV commercial font (curves are fine, but NOT comic sans, NOT overly rounded/bubbly), text tilted at slight upward angle. Single star burst effect ONLY on the "x" at the end of "dominAItrx". Below in white text: "Hire a human worker for your AI agents!" Retro TV commercial font. Authentic 80s synth music - upbeat and cheesy. This must be VERY BRIEF - exactly 1 second, no longer.

SCENE 2 (5 seconds): Static camera, locked-off angle. Cut to PERFECTLY AUTHENTIC ChatGPT interface - MUST include ChatGPT logo at top, MUST say "ChatGPT" text visible. Exact dark theme, exact teal accent colors, sidebar with ONLY 2 other chats visible (not many chats, just 2), distinctive chat bubble layout. NO dominAItrx branding visible. ONLY real ChatGPT interface elements. CRITICAL: Text must be CRISP, SHARP, and PERFECTLY READABLE - large clear font size. Realistic screen recording: mouse cursor visible, natural typing speed showing: "My best friend just told everyone my secret at dinner in front of 12 people. I'm humiliated and everyone is staring at me. What do I do right now?" User clicks send with cursor. HOLD on main ChatGPT interface. Then cut DIRECTLY to thinking indicator scene - NO extra cuts, NO zoom on prompt box, NO additional transitions. Music transitions to aspirational tech commercial soundtrack.
...
Rich cinematic color palette with cold blue warehouse tones contrasting warm skin tones. Professional smooth dolly cinematography. CRISP HIGH-RESOLUTION details on worker faces. NO subtitles except dominAItrx branding. NO breathing sounds, NO sniffing sounds. Background music: aspirational tech commercial soundtrack building to climactic crescendo at end, creating ironic contrast with dystopian visuals. The AI makes all decisions and delegates emotional labor to trapped human workers. Mood: disturbing corporate dystopia with falsely uplifting climactic music.`;
    ...
    if (status === 'completed') {
      ...
      const filename = `dominaitrx_v5_${video.id.slice(-8)}.mp4`;
...
console.log('🎬 dominAItrx V5 Video Ad Generator\n');
generateDominaitrxV5Ad();
